...
<script>
  export default {
    ...
    methods: {   //post the info to DB
      ...
      this.$http.put('http://'+process.env.VUE_APP_BOTURL+'/api/manufacturers/' + this.id, body).then();
      this.dialog = false
    },
    onDelete () {
    this.$emit('reloadmanufacturers')
    this.$http.delete('http://'+process.env.VUE_APP_BOTURL+'/api/manufacturers/' + this.id).then();
    this.dialog = false
    }
  }
}
