...
<script>
  export default {
    ...
    created() {
      this.$http.get('http://'+process.env.VUE_APP_BOTURL+'/api/ips').then(function(data){
        this.infos = data.body //get request data and send it do return items
    });
  },
  methods: {
    deleteItem (item) {
      this.$http.delete('http://'+process.env.VUE_APP_BOTURL+'/api/ips/'+item.id).then(function(){
        this.$http.get('http://'+process.env.VUE_APP_BOTURL+'/api/ips').then(function(data){
